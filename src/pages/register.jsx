      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-lg p-8">
          ...
          <div className="flex border-b border-gray-200 mb-6">
            <button className={`flex-1 py-3 text-center font-medium ${activeTab === 'login' ? 'text-[#6C47FF] border-b-2 border-[#6C47FF]' : 'text-gray-500'}`} onClick={switchToLogin}>
              登录
            </button>
            <button className={`flex-1 py-3 text-center font-medium ${activeTab === 'register' ? 'text-[#6C47FF] border-b-2 border-[#6C47FF]' : 'text-gray-500'}`} onClick={() => setActiveTab('register')}>
              注册
            </button>
          </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              field
                    ...
                    <FormLabel className="text-gray-700 text-sm">姓名</FormLabel>
                    <FormControl>
                      <div className="relative">
                        <User className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
                        <Input {...field} placeholder="请输入姓名" className="pl-10 pr-4 py-3 bg-gray-50 border-gray-200 focus:border-[#6C47FF] rounded-lg" />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>} />
          ...
          <div className="mt-6 text-center">
            <p className="text-gray-600 text-sm">
              已有账号？{' '}
              <button onClick={switchToLogin} className="text-[#6C47FF] hover:text-[#5A3BD9] font-medium underline">
                立即登录
              </button>
            </p>
          </div>
